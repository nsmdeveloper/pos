...
class Facturacion_Model extends CI_Model {
	...
	public function guardarFacturacion($cliente, $documento, $ncf, $tipo, $codigo, $nombre, $qty, $precio, $itbis, $orden){
		...
		for($i=0; $i < $count; $i++){
			...
			$insert_detalle = "INSERT INTO factura_detalle (id, numero_orden, codigo_producto, nombre_producto, precio, qty)
 	 			VALUES (NULL, '$orden', $codigo[$i], '$nombre[$i]', $precio[$i],  $qty[$i])
			";
	...
	public function agregarProducto($nombre, $descripcion,$categoria,  $precio, $existencia){

		$query = "INSERT INTO inventario (codigo, nombre, descripcion, categoria, precio, existencia) 
		 VALUES (NULL,'$nombre', '$descripcion', '$categoria', $precio, $existencia)";
	...
	public function editarProducto($codigo, $nombre, $descripcion, $categoria, $precio, $existencia, $estado){

		$query = "UPDATE inventario SET nombre = '$nombre', descripcion = '$descripcion', 
		categoria = '$categoria', precio = $precio, existencia = $existencia, estado = '$estado'
		WHERE codigo = $codigo";
	...
	public function printFacturaDetalle($orden){
		...
		$query = $this->db->query("SELECT cd.qty AS 'qty', i.codigo AS 'codigo', cd.precio AS 'precio', cd.nombre_producto AS 'nombre'
	...
	public function countFacturas(){
		...
		$query = $this->db->query("SELECT categoria FROM inventario
		WHERE categoria != '' GROUP BY categoria");
	...
	public function agregarCategoria($nombre, $descripcion){
		...
		$query = "INSERT INTO categoria (id, nombre, descripcion) 
	...
	public function editarCategoria($id, $nombre, $descripcion, $estado){
	...
	public function getCategoria(){
		...
		$query = $this->db->query("SELECT * FROM categoria ORDER BY nombre ASC");

		return $query->result_array();

	}

	public function getCategoria2($id){
